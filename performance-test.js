...
async function runPerformanceTest() {
	...
	try {
		...
		console.log(
			`   - Best Practices: ${Math.round(lhr.categories['best-practices'].score * 100)}/100`
		);
		...
		const bundleRequests = lhr.audits['resource-summary'].details.items.filter(
			(item) => item.resourceType === 'Script' || item.resourceType === 'Stylesheet'
		);
